...
struct threadList* mutexThreadLists[MAX_THREADS]; //indexed into by __align and contains linked list of threads waiting to obtain lock

int numOfThreads = 0; //used to track current number of threads and current position in tcbList

bool statusPrinter = 0;
...
static void schedule(int signal)
{
  ...
  if(tcbList[topThread].status == TS_EXITED){
	...
	if(topThread == numOfThreads+1){ //set back to index 1 if past max number of threads
      ...
      //printf("thread %d status = %d\n", topThread, tcbList[topThread].status);
